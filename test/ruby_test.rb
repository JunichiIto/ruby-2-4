...
class RubyTest < Minitest::Test
  ...
  def test_array_max_min
    ...
    a = [*1..10000]; 100000.times { assert 10000, a.max }
    a = [*1..10000]; 100000.times { assert 1, a.min }

    # no patch: 2.799s
    # Array#max defined: 1.224s
    x, y = 1, 2; 10000000.times { assert 2, [x, y].max }
  end
  ...
  def test_enumerable_uniq
    ...
    assert_equal [[1896, "Athens"], [1900, "Paris"], [1904, "Chikago"], [1908, "Rome"]], each_city_first_time
  ...
  def test_match_data_named_captures
    ...
    assert_equal({'year' => '2016', 'month' => '09', 'day' => '01'}, m.named_captures)
  end
  ...
  def test_logger_options
    ...
    logger = Logger.new(STDERR, level: :info, progname: :progname, formatter: formatter, datetime_format: "%d%b%Y@%H:%M:%S")
  ...
  def test_logger_shift_period_suffix
    require 'logger'
    # development.2016-09-01
    # development.2016-09-01.1
    # development.2016-09-01.2
    # のような名前のログファイルが出力される
    assert Logger.new(STDERR, shift_period_suffix: '%Y-%m-%d')
  end
  ...
  def test_date_time_to_time
    require 'date'
    assert_equal '2015-11-12 00:00:00 +0100', DateTime.strptime('2015-11-12 CET', '%Y-%m-%d %Z').to_time.to_s
  end

  # https://bugs.ruby-lang.org/issues/12271
  def test_time_to_time
    require 'date'
    assert_equal '2005-02-21 10:11:12 +0100', Time.new(2005, 2, 21, 10, 11, 12, '+01:00').to_time.to_s
  end
